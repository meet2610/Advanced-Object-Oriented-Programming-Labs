...
public class Turtle {
	 ...
	 * turtle will have an empty collection of commands. The starting position
	 * must be inside the square with corners {@code (0.0, 0.0)} and
	 * {@code (1.0, 1.0)}, otherwise an {@code IllegalArgumentException} will be
	 * thrown.
	 * 
	 * @param x
	...
	public Turtle(Turtle other) {
		...
		this.commands = new ArrayList<>(other.getCommands());
	...
	public void setPenColor(Color c) {
		...
		this.pen.setColor(c);
		TurtleCommand d = new TurtleCommandSetPenColor(this, old, c);
		this.commands.add(d);
	...
	/**
	 * Repeat all of the commands that this turtle has previously performed n
	 * times. For example:
	 * 
	 * <pre>
	 * Turtle t = new Turtle();
	 ...
	 * 
	 * @param n
	 *            the number of times to repeat the commands that this turtle
	...
	public void repeatCommands(int n) {
		...
		for (int j = 0; j < n; j++) {
			for (int i = 0; i < size; i++) {
				TurtleCommand c = this.commands.get(i);
				c.execute();
	 ...
	 * commands are executed in order that they appear in the given list, and
	 * are added to the end of this turtle's collection of commands.
	 * 
	 * @param commands
	...
	public void doCommands(List<TurtleCommand> commands) {
		...
		for (int i = 0; i < size; i++) {
			TurtleCommand c = commands.get(i);
			c.execute(this);
